...
<option class="code" value="0"<?php if($options['shrinkurl'] == 0) { echo " selected"; } ?>><?php _e('Do not shrink', 'twitpost'); ?></option>
<option class="code" value="1"<?php if($options['shrinkurl'] == 1) { echo " selected"; } ?>><?php _e('TinyURL', 'twitpost'); ?></option>
</select>
...
// Extra functions
// Call tinyurl API
function twitpost_calltinyurlAPI($url){
	// We're going to check that CURL exists, otherwise we'll just return the url
	if(!function_exists('curl_version'))
		return $url;
	
	$encodeURL = 'http://tinyurl.com/api-create.php?url='.urlencode($url);
    ...
    if(!preg_match('/^http:\/\/tinyurl.com\/([a-z]+)/', $result))
    	return $url;
    else
    	return $result;
}
